...
class Api_Match < FrozenRecord::Base

	self.base_path = 'data/'

	def self.num_picked(hid)
		numPicks = 0
		Api_Match.all.each do |match|
	...
	def self.hero_pick_rank(hid)
		rank = 1
		numPicked = Api_Match.num_picked(hid)
		Api_Hero.all.each do |hero|
			if Api_Match.num_picked(hero.hero_id) > numPicked
				rank += 1
			end
		end
		rank
	...
	def self.hero_ban_rank(hid)
		rank = 1
		numBanned = Api_Match.num_banned(hid)
		Api_Hero.all.each do |hero|
			if Api_Match.num_banned(hero.hero_id) > numBanned
				rank += 1
			end
		end
		rank
