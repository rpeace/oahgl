...
class Api_Hero < FrozenRecord::Base
	self.base_path = 'data/'

	def self.all_by_picks
		hero_ranks = Hash.new
		Api_Hero.all.each do |hero|
			hero_ranks[hero] = Api_Match.num_picked(hero.hero_id)
		end
	...
	def self.all_by_winrate
		hero_ranks = Hash.new
		Api_Hero.all.each do |hero|
			hero_ranks[hero] = Api_Match.win_percentage(hero.hero_id)
		end
		hero_ranks.keys.sort_by {|e| hero_ranks[e] }.reverse!
	end
	...
	def self.all_by_bans
		hero_ranks = Hash.new
		Api_Hero.all.each do |hero|
			hero_ranks[hero] = Api_Match.num_banned(hero.hero_id)
		end
		hero_ranks.keys.sort_by {|e| hero_ranks[e] }.reverse!
	end
